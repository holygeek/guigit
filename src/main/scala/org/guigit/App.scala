...
object App extends Application
{
  ...
  try {
    ...
    var repository = builder.setGitDir(new File("/home/nazri/src/git/.git")).readEnvironment()
    .findGitDir()
    .build();
    ...
    var a = 1
    log.call().iterator().foreach((commit:RevCommit) => { if (a < 2) { println(commit.getFullMessage()) }; a += 1 } )
  } catch {
